...
static void load_chan_rr_indices(INP int nodes_per_chan,
				 INP int chan_len,
				 INP int num_chans,
				 INP t_rr_type type,
				 INP t_seg_details * seg_details,
				 INOUTP int *index,
				 INOUTP t_ivec *** indices);
...
static int get_unidir_track_to_chan_seg(INP boolean is_end_sb,
					...
					INP t_seg_details * seg_details,
					INOUTP boolean * rr_edge_done,
					OUTP boolean * Fs_clipped,
					INOUTP struct s_linked_edge
					**edge_list);

static int vpr_to_phy_track(INP int itrack,
			    INP int chan_num,
			    INP int seg_num,
			    INP t_seg_details * seg_details,
			    INP enum e_directionality directionality);

static int *get_seg_track_counts(INP int num_sets,
				 INP int num_seg_types,
				 INP t_segment_inf * segment_inf,
				 INP boolean use_full_seg_groups);

static int *label_wire_muxes_by_type(INP int chan_num,
			     INP int seg_num,
			     INP t_seg_details * seg_details,
			     INP int max_len,
...
static int *label_wire_muxes(INP int chan_num,
			     INP int seg_num,
			     INP t_seg_details * seg_details,
			     INP int max_len,
			     INP enum e_direction dir,
...
static int *label_wire_muxes_for_balance(INP int chan_num,
					 ...
					 INP t_seg_details * seg_details,
					 INP int max_len,
					 INP enum e_direction direction,
					 INP int nodes_per_chan,
					 INP int *num_wire_muxes,
					 INP t_rr_type chan_type,
					 INP int *opin_mux_size,
					 INP t_ivec *** rr_node_indices);

static int *label_incoming_wires(INP int chan_num,
				 INP int seg_num,
				 INP int sb_seg,
				 INP t_seg_details * seg_details,
				 INP int max_len,
...
static int *
get_seg_track_counts(INP int num_sets,
		     INP int num_seg_types,
		     INP t_segment_inf * segment_inf,
		     INP boolean use_full_seg_groups)
{
...
t_seg_details *
alloc_and_load_seg_details(INOUTP int *nodes_per_chan,
			   INP int max_len,
			   INP int num_seg_types,
			   INP t_segment_inf * segment_inf,
			   INP boolean use_full_seg_groups,
 ...
 * started.                                                                  */
int
get_seg_start(INP t_seg_details * seg_details,
	      INP int itrack,
	      INP int chan_num,
	      INP int seg_num)
{
...
int
get_seg_end(INP t_seg_details * seg_details,
	    INP int itrack,
	    INP int istart,
	    INP int chan_num,
	    INP int seg_max)
...
int
get_bidir_opin_connections(INP int i,
			   INP int j,
			   INP int ipin,
...
int
get_unidir_opin_connections_by_type(INP int chan,
			    INP int seg,
			    INP int Fc,
			    INP t_rr_type chan_type,
			    INP t_seg_details * seg_details,
			    ...
			    OUTP boolean * Fc_clipped)
{
    ...
    *Fc_clipped = FALSE;
    ...
    inc_muxes = label_wire_muxes_by_type(chan, seg, seg_details, max_len,
				 INC_DIRECTION, nodes_per_chan,
				 &num_inc_muxes);
    dec_muxes =
    label_wire_muxes_by_type(chan, seg, seg_details, max_len, DEC_DIRECTION,
...
int
get_unidir_opin_connections(INP int chan,
			    INP int seg,
			    INP int Fc,
			    INP t_rr_type chan_type,
			    INP t_seg_details * seg_details,
...
static void
load_chan_rr_indices(INP int nodes_per_chan,
		     INP int chan_len,
		     INP int num_chans,
		     INP t_rr_type type,
		     INP t_seg_details * seg_details,
...
struct s_ivec ***
alloc_and_load_rr_node_indices(INP int nodes_per_chan,
			       INP int nx,
			       INP int ny,
			       INOUTP int *index,
			       INP t_seg_details * seg_details)
...
void
free_rr_node_indices(INP t_ivec *** rr_node_indices)
{
	int i, j, ofs;
 ...
 * transistor.                                                              */
int
get_track_to_tracks(INP int from_chan,
		    INP int from_seg,
		    INP int from_track,
...
static int
get_bidir_track_to_chan_seg(INP struct s_ivec conn_tracks,
			    INP t_ivec *** rr_node_indices,
			    INP int to_chan,
			    INP int to_seg,
			    INP int to_sb,
...
static int
get_unidir_track_to_chan_seg(INP boolean is_end_sb,
			     INP int from_track,
			     INP int to_chan,
			     INP int to_seg,
			     INP int to_sb,
...
static int
vpr_to_phy_track(INP int itrack,
		 INP int chan_num,
		 INP int seg_num,
		 INP t_seg_details * seg_details,
		 INP enum e_directionality directionality)
...
short *****
alloc_sblock_pattern_lookup(INP int nx,
			    INP int ny,
			    INP int nodes_per_chan)
{
...
void
free_sblock_pattern_lookup(INOUTP short *****sblock_pattern)
{
...
void
load_sblock_pattern_lookup(INP int i,
			   INP int j,
			   INP int nodes_per_chan,
			   INP t_seg_details * seg_details,
			   INP int Fs,
...
static int *
label_wire_muxes_by_type(INP int chan_num,
		 INP int seg_num,
		 INP t_seg_details * seg_details,
		 INP int max_len,
		 INP enum e_direction dir,
		 ...
		 OUTP int *num_wire_muxes)
{
    ...
    int *labels = NULL;
    boolean is_endpoint;
    int i, num_seg_types, cur_seg_type;
    int **labels_by_type;
    int *num_labels_by_type, *num_labels_by_type_2;

    num_seg_types = 0;
    for (i = 0; i < nodes_per_chan; i++) {
    	num_seg_types = max(seg_details[i].index, num_seg_types);
    }
    assert (num_seg_types == seg_details[nodes_per_chan-1].index);
    num_seg_types++;

    labels_by_type = (int *)my_malloc(sizeof(int *) * num_seg_types);
    num_labels_by_type = (int *)my_malloc(sizeof(int) * num_seg_types);
    for (i = 0; i < num_seg_types; i++) {
		num_labels_by_type[i] = 0;
	}
    num_labels_by_type_2 = (int *)my_malloc(sizeof(int) * num_seg_types);
    ...
    for(pass = 0; pass < 2; ++pass)
	{
	    ...
	    if(pass > 0)
		{
		    ...
		    for (i = 0; i < num_seg_types; i++) {
		    	labels_by_type[i] = my_malloc(sizeof(int *) * num_labels_by_type[i]);
		    }
		    for (i = 0; i < num_seg_types; i++) {
		    	num_labels_by_type[i] = 0;
			}
		...
		for(itrack = 0; itrack < nodes_per_chan; ++itrack)
		{
			...
			if(is_endpoint)
			{
				if(pass > 0)
				{
					labels_by_type[seg_details[itrack].index][num_labels_by_type[seg_details[itrack].index]] = itrack;
				}
				++num_labels_by_type[seg_details[itrack].index];
				num_labels++;
			}
		}
	}
	...
	cur_seg_type = 0;
	for (i = 0; i < num_seg_types; i++) {
		num_labels_by_type_2[i] = 0;
	}
	i = 0;
	while (i < num_labels) {
		if (num_labels_by_type_2[cur_seg_type] < num_labels_by_type[cur_seg_type]) {
			labels[i++] = labels_by_type[cur_seg_type][num_labels_by_type_2[cur_seg_type]];
			num_labels_by_type_2[cur_seg_type]++;
		}
		cur_seg_type = (cur_seg_type + 1) % num_seg_types;
	}
//    printf("[num_labels_after] ");
...
static int *
label_wire_muxes_for_balance(INP int chan_num,
			     INP int seg_num,
			     INP t_seg_details * seg_details,
			     INP int max_len,
			     INP enum e_direction direction,
...
static int *
label_wire_muxes(INP int chan_num,
		 INP int seg_num,
		 INP t_seg_details * seg_details,
		 INP int max_len,
		 INP enum e_direction dir,
